...
import { EvalResult, ImportBody } from '@holistics/aml/lib/src/types';
import { createInterpretResultError, createInterpretResultSuccess, getImportNodesFromAst, validateReturnValueOfModelAndDataset } from './utils';
import { convertAMLModelToModel } from './amlToModel';
import {
  AMLDataset, AMLJoinConfig, CurrentSelectedModel, Dataset, JoinConfig, Model, Relationship, InterpretResult,
} from '../../types/type';
import { Service } from '@holistics/aml';

export const validateRelationship = (amlRelationships: AMLJoinConfig[], dataModels: Model[]) => {
...
export const convertAMLRelationshipToRelationship = (amlRelationships: AMLJoinConfig[] = [], dataModels: Model[]): { relationships: Relationship[], joinConfigs: JoinConfig[] } => {
  validateRelationship(amlRelationships, dataModels);

  const relationships: Relationship[] = amlRelationships.map(amlRel => {
    const source_model = dataModels.find(model => model.name === amlRel.rel.from.model);
    const dest_model = dataModels.find(model => model.name === amlRel.rel.to.model);
  ...
  const joinConfigs: JoinConfig[] = zipWith(amlRelationships, relationships, (amlRelationship, relationship) => ({
...
export const convertAMLDatasetToDataset = (amlDataset: AMLDataset): Dataset => {
...
export const amlToDataset = ({ service }: {service: Service }, { filePath }: { filePath: string }): InterpretResult<Dataset> => {
  try {
    ...
    if (interpretResult.isOk()) {
      ...
      const dataset = convertAMLDatasetToDataset((evalResult.toJsStructure() as any).returnVal);
...
export const getCurrentSelectedModelsFromDataset = ({ service }, { filePath }: { filePath: string }): CurrentSelectedModel[] => {
  ...
  if (interpretResult.isOk()) {
    ...
    const modelNameAndPaths: CurrentSelectedModel[] = [];
    const importNodes = getImportNodesFromAst(ast);
    importNodes.forEach(node => {
      const importPath = vfs.pathResolver.resolveAml(
        vfs.pathResolver.resolve(
          vfs.pathResolver.dirname(
            vfs.pathResolver.toVfsPath(filePath).toString(),
          ).toString(),
          node.path.value,
        ).toString(),
      );
